...
@endsection
@section('title', __('admin/wishlist/index.title'))
@section('content')
<main class="flex column center">
  <h1 class="dark-blue">{{ __('admin/wishlist/index.subtitle') }}</h1>

  <div class="table-wrapper">
    <table>
      <thead>
        <tr>
          <th scope="col">{{ __('admin/wishlist/index.id') }}</th>
          <th scope="col">{{ __('admin/wishlist/index.name') }}</th>
          <th scope="col">{{ __('admin/wishlist/index.userId') }}</th>
          <th scope="col">{{ __('admin/wishlist/index.edit') }}</th>
          <th scope="col">{{ __('admin/wishlist/index.delete') }}</th>
        </tr>
      </thead>
      <tbody>
        @foreach ($viewData['wishlists'] as $wishlist)
        <tr>
          <td>{{ $wishlist->getId() }}</td>
          <td>{{ $wishlist->getName() }}</td>
          <td>{{ $wishlist->getUser()->getId() }}</td>
          <td>
            <a href="{{ route('admin.wishlist.edit', ['id' => $wishlist->getId()]) }}" class="btn-action">
          ...
          <td>
            <form action="{{ route('admin.wishlist.delete', $wishlist->getId())}}" method="POST">
              @csrf
              @method('DELETE')
  ...
  <div>{{ $viewData['wishlists']->links() }}</div>
  <div>
    <a href="{{ route('admin.wishlist.create') }}" class="btn-dark-blue">
      {{ __('admin/wishlist/index.create') }}
    </a>
  </div>
</main>
@endsection